...
$floorno = $roomname = $per_adult_price = $per_kid_price = $amenities = '';
if (isset($_GET['id'])) {
    ...
    if ($result && $result->num_rows > 0) {
        ...
        $per_adult_price = $row['per_adult_price'];
        $per_kid_price = $row['per_kid_price'];
...
if (isset($_POST["submit"])) {
    ...
    $stmt = $conn->prepare("UPDATE `tbl_rooms` SET `floorno`=?, `roomname`=?, `per_adult_price`=?, `per_kid_price`=?, `amenities`=? WHERE `id`=?");
    $stmt->bind_param("isddsi", $floorno, $roomname, $per_adult_price, $per_kid_price, $amenities, $id);
...
<div class="page-wrapper">
    ...
    <div class="row page-titles">
        <div class="col-md-5 align-self-center">
            <h3 class="text-primary">Edit Room</h3>
        </div>
    ...
    <div class="container-fluid">
        <div class="row justify-content-center">
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header">Update Room Details</div>
                    <div class="card-body">
                        <form class="form-valide" method="post" enctype="multipart/form-data" id="editRoomForm">
                            <!-- Floor Number -->
                            <div class="form-group">
                                <label for="floorno">Floor Number <span class="text-danger">*</span></label>
                                <div class="col-input">
                                    <input type="number" class="form-control" id="floorno" name="floorno" value="<?php echo htmlspecialchars($floorno); ?>" placeholder="Enter floor number (e.g., 1)" min="1" max="999" required>
                                    <div class="invalid-feedback">Please enter a valid floor number (1-999).</div>
                                </div>
                            </div>

                            <!-- Room Name -->
                            <div class="form-group room-name-group">
                                <label for="roomname">Room Type <span class="text-danger">*</span></label>
                                <div class="col-input">
                                    <select class="form-control" id="roomname" name="roomname" required>
                                        <option value="" disabled <?php if (empty($roomname)) echo 'selected'; ?>>Select Room Type</option>
                            ...
                            <!-- Per Adult Price -->
                            <div class="form-group">
                                <label for="per_adult_price">Price per Adult <span class="text-danger">*</span></label>
                                <div class="col-input">
                                    <input type="number" step="0.01" class="form-control" id="per_adult_price" name="per_adult_price" value="<?php echo htmlspecialchars($per_adult_price); ?>" placeholder="Enter price (e.g., 300.00)" min="0" max="999999.99" required>
                            ...
                            <!-- Per Kid Price -->
                            <div class="form-group">
                                <label for="per_kid_price">Price per Kid <span class="text-danger">*</span></label>
                                <div class="col-input">
                                    <input type="number" step="0.01" class="form-control" id="per_kid_price" name="per_kid_price" value="<?php echo htmlspecialchars($per_kid_price); ?>" placeholder="Enter price (e.g., 150.00)" min="0" max="999999.99" required>
                                    <div class="invalid-feedback">Please enter a valid price (0 or higher).</div>
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-input">
                                    <textarea class="form-control" id="amenities" name="amenities" placeholder="e.g., Wi-Fi, Fridge, TV"><?php echo htmlspecialchars($amenities); ?></textarea>
                                    <div class="helper-text">Enter amenities separated by commas (optional)</div>
                                    <div class="invalid-feedback">Please enter valid amenities.</div>
                                </div>
                            </div>
...
document.getElementById('editRoomForm').addEventListener('submit', function(event) {
    ...
    // Validate Amenities (optional, no validation needed unless specific rules apply)
    const amenities = form.querySelector('#amenities');
    if (amenities.value && amenities.value.length > 65535) { // MySQL text column max length
